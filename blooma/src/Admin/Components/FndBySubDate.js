...
import { Subdate } from './../../Redux/index';
...
const FndBySubDate = () => {
    ...
    const Data = useSelector(state => state.Admin.subdate);
    const dispatch = useDispatch();
    const [date , setDate] = useState()

    const HandleSubmitDate = (e) =>{
        e.preventDefault();
        axios.post(`/fndcnsumrbysbscrptiondate`,{"date" : date},{
            headers:{
        ...
        .then(res=>{
            ...
            dispatch(Subdate(res.data.consumers))
        })
        .catch(err=>{
    ...
    return (
        <div>
            <h2>Find Consumer by Subscription date</h2>
            <form onSubmit={HandleSubmitDate}>
                <input type="text" required onChange={e=>setDate(e.target.value)} placeholder="Enter Date"/>
